(function () {
    ...
    function BankAccountsCtrl($scope,environmentConfig,$uibModal,toastr,$http,cookieManagement,errorToasts,$window,errorHandler) {
        ...
        vm.getBankAccount = function (bankAccount) {
            $scope.loadingBankAccounts = true;
            $http.get(environmentConfig.API + '/user/bank-accounts/' + bankAccount.id + '/', {
                headers: {
        ...
        $scope.updateBankAccount = function () {
            ...
            $scope.editingBankAccounts = !$scope.editingBankAccounts;
            $scope.loadingBankAccounts = true;
            $http.patch(environmentConfig.API + '/user/bank-accounts/'+ $scope.editBankData.id + '/', vm.updatedBankAccount, {
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': vm.token
                }
